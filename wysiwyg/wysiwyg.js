    ...
    var create_editor = function ($textarea, classes, placeholder, toolbar_position, toolbar_buttons, toolbar_submit, label_selectImage,
      ...
      var content_insertimage = function (wysiwygeditor) {
        ...
        var $fileuploader = null,
          $fileuploader_input = $('<div class="dropimage">');
        $fileuploader = $('<div/>').append($fileuploader_input);
        $('<div/>').addClass('wysiwyg-browse').addClass('dropimage')
          .appendTo($content);
      ...
      var content_insertiframe = function (wysiwygeditor) {
        // Add video to editor
        var insert_iframe_wysiwyg = function (url, html) {
          url = $.trim(url || '');
          html = $.trim(html || '');
          var website_url = false;
          if (url.length && !html.length)
            ...
            html.match(/^(?:https?:\/)?\/?(?:[^:\/\s]+)(?:(?:\/\w+)*\/)(?:[\w\-\.]+[^#?\s]+)(?:.*)?(?:#[\w\-]+)?$/))
            website_url = html;
          html = iframe_from_url(website_url) || '';
          wysiwygeditor.insertHTML(html).closePopup().collapseSelection();
        };
        // Create popup
        var $content = $('<div/>').addClass('wysiwyg-toolbar-form')
        ...
        var $inputurl = $('<input type="text" value="">').addClass('wysiwyg-input')
          .keypress(function (event) {
            if (event.which == 10 || event.which == 13)
              insert_iframe_wysiwyg($inputurl.val());
          });
        if (placeholder_url)
          $inputurl.prop('placeholder', placeholder_url);
        var $okaybutton = $();
        if (toolbar_submit)
          $okaybutton = toolbar_button(toolbar_submit).click(function (event) {
            insert_iframe_wysiwyg($inputurl.val(), $textareaembed.val());
            event.stopPropagation();
            event.preventDefault();
            return false;
          });
        ...
        switch (name) {
          case 'insertimage':
            ...
            return function (target) {
              ...
              var dropzone = new Dropzone($('.dropimage')[0], {
                "url": img_url,
                "uploadMultiple": false,
                "success": _.bind(function (file, ret) {
                  insert_image_wysiwyg(ret.uri, _.uniqueId('image_'), wysiwygeditor);
                }, this)
              });
      ...
      if (!option || typeof(option) === 'object') {
        ...
        return this.each(function () {
          ...
          var classes = option.classes,
            ...
            iframe_from_url = option.iframeFromUrl || null,
